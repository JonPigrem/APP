...
float sSortedTouches[NUM_SENSORS][MAX_TOUCHES];	// Touch location for each finger on each sensor
	...
	while(!gShouldStop)
	{
		...
		for (int i = 0; i < NUM_SENSORS; i++) {
			...
			if(Sensor[i].readSensors() == 0) {
				...
				if (loopIndex == 13) {
					loopIndex = 0;
					printf("\nSENSOR %d  \tTouches: %d  Last touch:%d\t", i + 1, sTouchNum[i], sLastIndex[i] + 1);
					for(int j = 0; j < sTouchNum[i]; j++)
