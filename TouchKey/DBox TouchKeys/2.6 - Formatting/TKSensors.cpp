...
int TKSensors::initSensors(int tk_bus, int tk_address, int tk_file, int sensorTypeToUse)
{
	...
	if(tk_address >= 0) {
		if(TK[0].initI2C_RW(tk_bus, tk_address, tk_file)>0)
			return 1;
	...
	if(tk_address >= 0) {
		if(TK[1].initI2C_RW(tk_bus, tk_address, tk_file)>0)
			return 1;
...
int TKSensors::readSensors()
{
	...
	if(TK[0].ready()) {
		...
		TKTouchCount[0] = TK[0].getTouchCount();
	}
	else
		TKTouchCount[0] = 0;
	...
	if(TK[1].ready()) {
		...
		TKTouchCount[1] = TK[1].getTouchCount();
	}
	else
		TKTouchCount[1] = 0;
	...
	if(TKTouchCount[0] == 0 && TKTouchCount[1] == 0)
		resetSensorsData();
	...
	else
	{
		...
		{
			TKXPositions[0][i] = TK[0].getSliderPosition()[i];
			TKTouchSize[0][i] = TK[0].getSlidersize()[i];

			TKXPositions[1][i] = TK[1].getSliderPosition()[i];
			TKTouchSize[1][i] = TK[1].getSlidersize()[i];
		}
		// touch position Y
		TKYPosition[0] 	 = TK[0].getSliderPositionH();
		TKYPosition[1] 	 = TK[1].getSliderPositionH();
	}
...
{
	int tk_file			= 0;
	// Indicate each tk_bus as 1 regardless of the size of the array
