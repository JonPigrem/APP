...
#define NUM_SENSORS 4 // Number of sensors used (default = 2)
#define MAX_TOUCHES 1 // Maximum number of touches on each sensor. Depends on sensor type (default = 5)
...
void render(BelaContext *context, void *userData)
{
    ...
    for (int i = 0; i < NUM_SENSORS; i++) {
        ...
        // Number of finger touches for each sensor
        const char *name = "N";
        char str[32] = {0};
        sprintf(str, "%s%d", name, i + 1);  // N1 for sensor 1, etc.
        libpd_float(str, sTouchNum[i]);
        
        // Last finger pressed on each sensor
        name = "L";
        sprintf(str, "%s%d", name, i + 1);  // L1 for sensor 1, etc.
        libpd_float(str, sLastIndex[i] + 1);
        
        // Overall Y touch location on each sensor
        name = "Y";
        sprintf(str, "%s%d", name, i + 1);  // Y1 for sensor 1, etc.
        libpd_float(str, sYTouches[i]);
                    
        for (int j = 0; j < MAX_TOUCHES; j++) {
        
            // X touch location for each finger on each sensor
            name = "X";
            sprintf(str, "%s%d.%d", name, i + 1, j + 1);  // X1.3 for sensor 1, finger 3, etc.
            libpd_float(str, sSortedTouches[i][j]);
            
            // Touch size for each finger on each sensor
            name = "S";
            sprintf(str, "%s%d.%d", name, i + 1, j + 1);  // S1.3 for sensor 1, finger 3, etc.
            libpd_float(str, sSize_[i][j]);
    ...
    //// FOR EACH SENSOR i, THE DATA SENT OUT TO PURE DATA IS ////
        ...
        // Li       for last finger pressed
        // Yi       for overall Y touch location
        // Xi.1     for X location of finger 1
        // Xi.2     for X location of finger 2
        // Xi.3     for X location of finger 3
        // Si.1     for touch size of finger 1
        // Si.2     for touch size of finger 2
        // Si.3     for touch size of finger 3
