...
int I2c_TouchKeyJP::initTouchKey(int sensorTypeToUse)
{
	...
	//------------
	char buf2[4] = { 0x00, 0x02, 0x00, 0xC}; // change bit depth 9-12bit 0x9 - 0xC
	if(write(i2C_file, buf2, 4) !=4)
	{
		cout << "Failed to set TouchKey resolution" << endl;
		return 1;
	}
...
int I2c_TouchKeyJP::readI2C()
{
	...
	*/
	...
	if(sensorType == kSensorTypeDBox1) {
		...
		rawSliderPositionH = ((dataBuffer[16] << 8) + dataBuffer[17]) & 0x0FFF;
	}
	else {
		rawSliderPosition[0] = (((dataBuffer[0] & 0xF0) << 4) + dataBuffer[1]);
		rawSliderPosition[1] = (((dataBuffer[0] & 0x0F) << 8) + dataBuffer[2]);
	...
	for(int i = 0; i < 4; i++)
	{
		...
		if(rawSliderPosition[i] != 0x0FFF) // 0x0FFF means no touch
		{
			if(sensorType != kSensorTypeTouchKey){
				...
				storedSliderPosition[i] = sliderPosition[i]; 
		}	else
				sliderPosition[i] = (float)rawSliderPosition[i] / 1792.0;	
			...
			if(sensorType == kSensorTypeDBox2) {
				sliderSize[i]     = (float)((dataBuffer[i + 8] << 8) + dataBuffer[i + 9]) / 5000.0;
				if(sliderSize[i] > 1.0)
					sliderSize[i] = 1.0;
			}
			...
			sliderPosition[i] = storedSliderPosition[i]; // change from -1
			sliderSize[i]     = 0.0;
		}
	}


	int storedSliderPositionH;
	if(rawSliderPositionH != 0x0FFF)
	{
		sliderPositionH = (float)rawSliderPositionH / 1792.0; // White keys, horizontal (1 byte + 1 bit)
		storedSliderPositionH = sliderPositionH;
	}
	else
		sliderPositionH = storedSliderPositionH; // change from -1
