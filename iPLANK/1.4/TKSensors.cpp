...
int TKSensors::readSensors() {
	...
	//			printf("XPos1: %f  ", TK[i].getSliderPosition()[0]);
	//			printf("XPos2: %f  ", TK[i].getSliderPosition()[0]);
...
void TKSensors::resetSensorsData() {
	for (int i = 0; i < NUM_SENSORS; i++) {
		for(int j=0; j<MAX_TOUCHES; j++) {
			//TKXPositions[i][j] = -1; // commented out to prevent filter snapping to new position on finger off
			//TKYPosition[i]	 = -1; // commented out to prevent filter snapping to new position on finger off
			TKTouchSize[i][j] = 0;
		}
	}
	return;
